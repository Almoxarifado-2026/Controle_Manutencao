...
### 4.1 (Opcional) Notificação por e-mail, via Resend
Além do push, dá pra mandar os mesmos avisos por e-mail para uma lista de
destinatários — isso é configurado direto no app (⚙️ Configuração →
Notificações → "📧 Enviar por e-mail também"), mas o envio de verdade
depende de cadastrar uma chave do [Resend](https://resend.com) (tem plano
gratuito, sem cartão):

1. Crie uma conta grátis em resend.com
2. No painel do Resend, vá em **API Keys → Create API Key** e copie a chave
3. Cadastre no Supabase:
```bash
supabase secrets set RESEND_API_KEY="re_SuaChaveAqui"
```
Pronto — sem mais nenhuma configuração, os e-mails já saem de
`onboarding@resend.dev` (remetente de testes que o Resend libera sem
precisar verificar domínio, com um pequeno limite diário — suficiente para
uso normal deste app). Se no futuro vocês quiserem que o remetente seja do
domínio de vocês (ex: `avisos@suaempresa.com.br`), verifiquem o domínio no
painel do Resend e depois rodem:
```bash
supabase secrets set EMAIL_REMETENTE="Controle de Manutenção <avisos@suaempresa.com.br>"
```
Se `RESEND_API_KEY` não for cadastrada, o envio de e-mail é simplesmente
pulado (o push continua funcionando normalmente).
