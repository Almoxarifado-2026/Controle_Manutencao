...
Deno.serve(async (_req: Request) => {
  ...
  if (!config.dias_semana.includes(diaSemana) || !dentroDaJanela(config.horario, minutosDoDia)) {
    return new Response(
      JSON.stringify({
        ok: true,
        avisos: 0,
        motivo: `fora do dia/horário configurado (agora: dia ${diaSemana} ${horaMinuto}, configurado: dias ${config.dias_semana.join(",")} às ${config.horario})`,
      }),
      { headers: { "Content-Type": "application/json" } }
    );
  }
